...
from llm.ollama_helpers import _is_host_reachable, get_available_ollama_host
...
def test_is_host_reachable_returns_true_on_success():
    ...
    with patch("llm.ollama_helpers.urllib.request.urlopen"):
...
def test_is_host_reachable_returns_false_on_url_error():
    ...
    with patch(
        "llm.ollama_helpers.urllib.request.urlopen",
...
def test_is_host_reachable_passes_timeout():
    ...
    with patch("llm.ollama_helpers.urllib.request.urlopen") as mock_urlopen:
...
def test_returns_remote_when_configured_and_reachable():
    ...
    with patch("llm.ollama_helpers._is_host_reachable", return_value=True):
...
def test_does_not_ping_when_remote_not_configured():
    ...
    with (
        ...
        patch("llm.ollama_helpers._is_host_reachable") as mock_ping,
    ):
