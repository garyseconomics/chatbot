...
- [ ] **Improve Ollama configuration for chatbot and embedding models** — Currently embeddings always use the local Ollama host, while the chatbot LLM supports remote/local with fallback. Unify the configuration so both chatbot and embedding models can use remote or local Ollama, with consistent fallback behavior. Consider adding `use_remote_embeddings`, a remote embedding host env var, and fallback logic in `vector_database_manager.py`.
