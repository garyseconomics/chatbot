...
- [x] **Improve error handling in Discord and Telegram bots** -- (1) Discord: replaced hardcoded error string with `settings.error_messages["DefaultError"]`; (2) Telegram: added try/except to `question()` handler matching Discord's pattern. Note: `RAG_query` already catches LLM/provider errors internally, so bot-level handlers only catch failures outside `RAG_query` (e.g., platform API errors when sending messages). Specific `HTTPException` handling deferred until #33 (message length) is addressed.
