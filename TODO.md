...
- [ ] **Embedding model and provider decisions** ([#40](https://github.com/garyseconomics/chatbot/issues/40)) -- Reviewed 2026-03-22: `qwen3-embedding:8b` (MTEB 70.58) is still the best embedding model available on Ollama. The only models scoring higher — NVIDIA's NV-Embed-v2 (72.31) and Llama-Embed-Nemotron-8B — are not on Ollama yet. Ollama Cloud does not support any embedding models as of this date. Cloud providers (OpenRouter, SiliconFlow, DeepInfra) offer this model cheaply but require OpenAI-compatible API support (depends on #41). Re-check in mid-2026.
  - [x] **Investigate why `user_id` is null in vector_search traces** -- The `@observe(name="vector_search")` decorator on `retrieve()` in `rag_manager.py` creates a separate Langfuse trace with `userId: null`. The real user_id is only available inside the function's input args, not at the trace level. The parent trace (from `chat()`) has the correct user_id, but the vector_search trace doesn't inherit it. Investigate whether Langfuse's `@observe` can propagate user_id from the parent trace, or whether we need to call `update_current_trace()` inside `retrieve()`.
  - [x] **Record embedding model name and provider in vector_search Langfuse traces** -- Currently the `@observe` decorator on `retrieve()` doesn't record which embedding model or provider was used. Add `model` and `provider` to the trace metadata (similar to how `chat()` records them via `update_current_trace()`). The embedding model has been `qwen3-embedding:8b` since before we started recording Langfuse traces.
  - [ ] **Improve `analytics/export.py` to classify vector_search traces** -- Currently `classify_traces()` splits traces into user and other. Vector_search traces (1,275 of them in our March 22 export) end up in `other_traces` mixed with evaluator traces. Update the export to produce four categories: user traces, vector_search traces, evaluator traces, and other traces. Also create an importer (`analytics/vector_search_importer.py`) to import vector_search traces into their own SQLite table (`vector_search_traces`).
  - [ ] **Benchmark embedding latency per provider** -- Depends on "Record embedding model name and provider" and "Improve analytics/export.py" above. Run two dedicated battery tests that only exercise the vector search step (skip the chat model entirely). (1) Run against `ollama_self_hosted` only — set `embedding_provider_priority` to `["ollama_self_hosted"]`. (2) Run against `ollama_local` only — set `embedding_provider_priority` to `["ollama_local"]`. Compare results to get clean, isolated latency numbers for each provider. Plan: `testing_phase_1/embedding_latency_plan.md`.
  - [ ] **Compare benchmark results with Phase 1 historical data** -- Phase 1 embedding latency is already documented in `testing_phase_1/latency_report_embedding.md` (1,275 traces, March 10–21). After the per-provider benchmark is complete, compare those clean isolated numbers with the historical data and update the report.
...
- [x] **Analyze user traces** ([#32](https://github.com/garyseconomics/chatbot/issues/32)) -- Exported traces from Langfuse, imported into SQLite, and analyzed with Claude Code. This replaced the original plan of using Langfuse dashboards. Chat model latency report completed in `testing_phase_1/latency_report_chat_model.md`. Embedding latency report started in `testing_phase_1/latency_report_embedding.md` — remaining work (per-provider comparison) moved to [#40](https://github.com/garyseconomics/chatbot/issues/40).
