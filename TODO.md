...
- [ ] **Retrieve latest Q&A and identify bad answers from prompt v3** -- Export user traces from Langfuse for the period between the v3 prompt deployment (2026-03-21 04:13) and the v3 fix deployment (2026-03-23 19:18). Deployment times confirmed via Discord bot greeting messages. The v3 prompt was refusing to answer some economics questions when the reference material had no relevant information. Traces from this period are not yet in the analytics database (latest imported trace is 2026-03-21 14:12, prompt_version "2"). Steps: (1) Export traces from Langfuse for 2026-03-21 04:13 to 2026-03-23 19:18; (2) Import them into the `user_traces` table with prompt_version "3"; (3) Review the answers and identify cases where the bot refused to answer or gave a bad answer to a legitimate economics question.
- [ ] **Update README.md with current setup instructions** -- The README has outdated paths (e.g., `chatbot/vector_database/chroma_langchain_db/` instead of `data/`), references old config options, and is missing deployment instructions for the new server setup. Update to reflect: (1) current project structure after the reorganization; (2) current database path and how to get a copy from [garyseconomics/chatbot-database](https://github.com/garyseconomics/chatbot-database); (3) clearer migration/deployment steps for someone on Gary's team (the `scp` command assumes SSH access — provide simpler alternatives or more detailed instructions); (4) server security hardening basics (SSH keys, firewall). Also explain that shared variables (`EMBEDDINGS_MODEL`, `COLLECTION_NAME`, `DATABASE_PATH`, `VIDEO_IDS_SEPARATOR`) are now configured in `.env` so they stay in sync between the main application (`src/config.py`) and the content database import scripts (`content_database/config.py`). Update the Testing section to explain how to run importer tests separately: `pytest content_database/scripts/tests/` (main app tests stay as `pytest`, which only runs `tests/`).

## Telegram and Discord bot fixes
...
- [ ] **Review and test the content database importer scripts** -- The scripts in `content_database/scripts/` were moved from `src/` and imports were updated, but they haven't been tested yet. Tasks: (1) remove the `LLM_Client` dependency from `vector_database_manager.py` — create a local function to get the embeddings model; (2) verify all scripts run correctly (`import_documents.py`, `collections_viewer.py`); (3) run the tests in `content_database/scripts/tests/` and fix any failures.
...
- [ ] **Add more content to the vector database** -- The current database only includes video subtitles from early 2024 to November 2025. Several important sources are missing. See `prompt_issues/prompt_issues.md` (Source Material Gaps) for the full list.
  ...
  - [ ] Clean up and import transcripts from [garyseconomics/transcripts](https://github.com/garyseconomics/transcripts/tree/main/transcripts) ([#39](https://github.com/garyseconomics/chatbot/issues/39)) — these need review before they can be used. Clean subtitles should be added to [garyseconomics/subtitle-data](https://github.com/garyseconomics/subtitle-data). See `docs/missing_subtitles.txt` for the full list of 310 missing transcripts (261 full videos + 49 shorts).
